...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    if (!this.currentVariant) {
      ...
      this._filterVariateThumbnail(this.currentVariant);
  ...
  _filterVariateThumbnail(variant) {
    if (variant.featured_image != null) {
      // いったんすべてを隠す
      $("[data-variant-thumbnail]").hide();

      // variant_imageをフィルタリングして表示する（data-variant-thumbnail="alt"）
      const selected_color = variant.featured_image.alt;
      console.log(variant.featured_image.alt);
      const thumbnail_selector_variant = `[data-variant-thumbnail="${selected_color}"]`;
      $(thumbnail_selector_variant).show();
  ...
  _renderProductInfo() {
    fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.section}`)
      .then((response) => response.text())
      .then((responseText) => {
        const id = `status-${this.dataset.section}`;
        const html = new DOMParser().parseFromString(responseText, "text/html");
        const destination = document.getElementById(id);
        const source = html.getElementById(id);
        ...
        const price = document.getElementById(`status-${this.dataset.section}`);
        if (price) price.classList.remove("visibility-hidden");
        this.toggleAddButton(!this.currentVariant.available, window.variantStrings.soldOut);
      });
  }
